...
if(rs.next()) {
	...
	ArrayList<Solution_entry> solutions = ms.getEligibleSolutions(myDBBean, answerCopy, type);
	
	String absPath = new java.io.File(application.getRealPath("solution.jsp")).getParent();	
    ...
    <div class="row">      
      ...
      <b><font color=black size=4 face="arial">Case Summary:</font></b>
      	<table width=100%>
      		...
      		<tr><td style="line-height:18px"><font size=3 color=#004B97 face="arial">
      		<%
  	...
  	<div class="8u"> 
  		...
  		<div align=center>
  			...
  			<font size=3 color=#004B97 face="arial"> solutions were recommended for your case</font>
  			<br />
  			<font size=3 color=black face="arial"><i>Review by:</i> </font>
  			<select id="present" name="present" onchange="changetopic(this.value);">
      			<option value="contributing_factor" selected="selected">Contributing Factors</option>
      			<option value="solution_type">Solution Types</option>      		
      		</select>
     	...
     	<%int factorCount = 0;
     	for (Entry<String, Integer> entry : contributingFactor.entrySet()) { 
     		factorCount ++; 
     	%>
     		<tr>
     			<td>
     				<a style="text-decoration:none" id="cf_<%=factorCount %>" href="javascript:show_hiddendiv('cf_<%=factorCount%>');">
     					<span id="span_cf_<%=factorCount%>">
     						<img src="images/plus.png" title="Click to review all <%=entry.getValue() %> solutions belong to this contributing factor" height=20>
     					</span>
     				</a>
     			</td>
     			<td colspan = "3">
     				<font size=4 color=brown face="arial"><%=entry.getKey().replaceAll("NA", "Others") %> (<font size=4 color=darkorange face="verdana"><%=entry.getValue() %></font>)</font>
     			</td>
     		</tr>
     		<tr>
     			<td></td>
     			<td>
     				<span style="display:none" id="cf_<%=factorCount %>_details">
     					<table>     						
     						<% for (int i=0; i<solutions.size(); i++) {
     							if(solutions.get(i).Category.equals(entry.getKey())) {%>
     								<tr>     										
     									<td><b><font size=3 color=black face="arial">&bull;</font></b></td>
     									<td><b><font size=3 color=black face="arial"><%=solutions.get(i).Solution %></font></b></td>
		...
		<table id="solution_type" style="display:none">    	
     	<%int typeCount = 0;
     	for (Entry<String, Integer> entry : solutionType.entrySet()) { 
     		typeCount ++; %>
     		<tr>
     			<td>
     				<a style="text-decoration:none" id="st_<%=typeCount %>" href="javascript:show_hiddendiv('st_<%=typeCount%>');">
     					<span id="span_st_<%=typeCount%>">
     						<img src="images/plus.png" title="Click to review all <%=entry.getValue() %> solutions belong to this solution type" height=20>
     					</span>
     				</a>
     			</td>
     			<td colspan = "3">
     				<font size=4 color=brown face="arial">
     					<%if(entry.getKey().equals("0")) {%>
     						Principle(<font size=4 color=darkorange face="verdana"><%=entry.getValue() %></font>) <img src="images/question_mark.jpg" title="Unspecific actions, may be actionable for managers" height=15>
     					<%} %>
     					<%if(entry.getKey().equals("1")) {%>
     						Direct action(<font size=4 color=darkorange face="verdana"><%=entry.getValue() %></font>) <img src="images/question_mark.jpg" title="For clinical staff to make specific actions" height=15>
     					<%} %>
     					<%if(entry.getKey().equals("2")) {%>
     						Patients(<font size=4 color=darkorange face="verdana"><%=entry.getValue() %></font>) <img src="images/question_mark.jpg" title="Patient actions" height=15>
     					<%} %>
     					<%if(entry.getKey().equals("NA")) {%>
     						Others(<font size=4 color=darkorange face="verdana"><%=entry.getValue() %></font>)
     					<%} %>     					
     				</font>
     			</td>
     		</tr>
     		<tr>
     			<td></td>
     			<td>
     				<span style="display:none" id="st_<%=typeCount %>_details">
     					<table>     						
     						<% for (int i=0; i<solutions.size(); i++) {
     							if(solutions.get(i).Solution_type.equals(entry.getKey())) {%>
     								<tr>     										
     									<td><b><font size=3 color=black face="arial">&bull;</font></b></td>
     									<td><b><font size=3 color=black face="arial"><%=solutions.get(i).Solution %></font></b></td>
     									<td><img src="images/like.png" title="This solution is helpful" height=20>&nbsp;<img src="images/dislike.png" title="This solution is not helpful" height=20></td>
